...
<div class="content1">
    <div class="com-hd">
        <p><?php echo html_escape(element('board_name', element('board', $view))); ?> 글쓰기</p>
    </div>
	...
	?>
		...
		<ul class="com-cont-ul">
			<?php if (element('is_post_name', element('post', $view))) { ?>
				<li class="cont-ul-tit">
					<input type="text" class="input px150" name="post_nickname" id="post_nickname" value="<?php echo set_value('post_nickname', element('post_nickname', element('post', $view))); ?>" placeholder="이름"/>
				</li>
				<?php if ($this->member->is_member() === false) { ?>
					<li class="cont-ul-tit">
						<input type="password" class="input px150" name="post_password" id="post_password" placeholder="비밀번호"/>
					</li>
				<?php } ?>
				<li class="cont-ul-tit">
					<input type="text" class="input px400" name="post_email" id="post_email" value="<?php echo set_value('post_email', element('post_email', element('post', $view))); ?>" placeholder="이메일"/>
				</li>
				<li class="cont-ul-tit">
					<input type="text" class="input px400" name="post_homepage" id="post_homepage" value="<?php echo set_value('post_homepage', element('post_homepage', element('post', $view))); ?>" placeholder="홈페이지"/>
				</li>
			<?php } ?>
			<li class="cont-ul-tit">
                <input type="text" class="input px400" name="post_title" id="post_title" value="<?php echo set_value('post_title', element('post_title', element('post', $view))); ?>" placeholder="제목"/>
			...
			<?php if (element('use_subject_style', element('board', $view))) { ?>
				<li class="cont-ul-tit">
					...
					<label class="checkbox-inline" for="post_title_color">
						색상 : <input type="text" class="input px100" name="post_title_color" id="post_title_color" value="<?php echo set_value('post_title_color', element('post_title_color', element('meta', element('post', $view))) ? element('post_title_color', element('meta', element('post', $view))) : '#000000'); ?>" />
			...
			?>
				<li class="cont-ul-tit">
					<input type="text" class="input px400" name="<?php echo $link_column; ?>" value="<?php echo set_value($link_column, $link); ?>" placeholder="링크 #<?php echo $i+1; ?>"/>
				</li>
			<?php
				}
			}
			...
			<?php if (element('use_post_tag', element('board', $view)) && element('can_tag_write', element('board', $view))) { ?>
				<li>
					<span>태그</span>
					<input type="text" class="input px400" name="post_tag" id="post_tag" value="<?php echo set_value('post_tag', element('post_tag', element('post', $view))); ?>" />
					<div class="help-block" style="padding-left:250px;">태그를 콤마(,)로 구분해 입력해주세요. 예) 자유,인기,질문</div>
				</li>
			<?php } ?>
			<?php
			...
			?>
				...
				<div class="post_poll_area" <?php if ( ! element('poll_item', $view)) { ?>style="display:none;" <?php } ?> >
					<li>
						...
						<div class="help-block" style="padding-left:300px;">기간을 입력하지 않으시면, 기간제한없이 참여 가능합니다</div>
					</li>
					<li>
						<span>설문제목</span>
        ...
        <div class="btn-box">
            <button type="button" class="blue-b-btn2">취소</button>
            <button type="submit" class="blue-btn">작성완료</button>
        </div>
	<?php echo form_close(); ?>
</div>
